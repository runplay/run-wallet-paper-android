...
public class Kerl extends JCurl {
    ...
    private static int bigint_cmp(final int[] lh, final int[] rh) {
        for (int i = INT_LENGTH - 1; i >= 0; i--) {
            // changed below as Long.compare on availbale Android 19, this is Android 15
            //int ret = Long.compare(Kerl.toUnsignedLong(lh[i]), Kerl.toUnsignedLong(rh[i]));
            Long l1=Long.valueOf(Kerl.toUnsignedLong(lh[i]));
            int ret = l1.compareTo(Long.valueOf(Kerl.toUnsignedLong(rh[i])));
            if (ret != 0) {
                return ret;
            }
        }
